...
def load_data():
    global data
    try:
        f = open("data_debug.txt", "r", encoding="utf8")
...
def save_data():
    ...
    with open("data_debug.txt", "w", encoding="utf8") as f:
...
def on_receive_callback(message: telebot.types.Message):
    ...
    if user_data.state == "EnterPassword":
        ...
        if entered_pass == password:
            ...
            if user_data.towers == 4:
                bot.send_message(message.chat.id, "🎉 Поздравляем, вы прошли игру, в качестве подарка дарим вам стикеры: https://t.me/addstickers/LukoilOilCam")
            else:
        ...
        if user_data.state == "StartScreen":
            return bot.send_message(message.chat.id, "Вы должны устроиться на работу")
        earned_money = random.randint(300, 500)
        user_data.money += earned_money
        user_data.click_count += 1
        save_data()
        ...
        if random.randint(0, 10) >= 9:
            ...
            earned_money = random.randint(500, 800)
